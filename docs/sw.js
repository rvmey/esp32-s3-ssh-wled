...
self.addEventListener('fetch', event => {
  ...
  if (reqUrl.pathname.endsWith('/manifest-cyd.json')) {
    const target = 'https://raw.githubusercontent.com/rvmey/esp32-s3-ssh-wled/master/docs/manifest-cyd-2.0.424.json';
    event.respondWith(fetch(target, { cache: 'no-store' }));
    return;
  }
  ...
  if (
    ...
    const target = new URL('/esp32-s3-ssh-wled/firmware/esp32_cyd_picture_frame-2.0.424.bin', self.location.origin);
    event.respondWith(fetch(target.toString(), { cache: 'no-store' }));
    return;
  }
