...
self.addEventListener('fetch', event => {
  ...
  if (reqUrl.pathname.endsWith('/manifest-core2.json')) {
    const target = 'https://raw.githubusercontent.com/rvmey/esp32-s3-ssh-wled/master/docs/manifest-core2-2.0.264.json';
    event.respondWith(fetch(target, { cache: 'no-store' }));
    return;
  }

  if (reqUrl.pathname.endsWith('/manifest-picture_frame.json')) {
    const target = 'https://raw.githubusercontent.com/rvmey/esp32-s3-ssh-wled/master/docs/manifest-picture_frame-2.0.264.json';
    event.respondWith(fetch(target, { cache: 'no-store' }));
    return;
  }
  ...
  if (
    ...
    const target = new URL('/esp32-s3-ssh-wled/firmware/esp32_core2_picture_frame-2.0.264.bin', self.location.origin);
  ...
  if (
    ...
    const target = new URL('/esp32-s3-ssh-wled/firmware/esp32_picture_frame-2.0.264.bin', self.location.origin);
    event.respondWith(fetch(target.toString(), { cache: 'no-store' }));
  }
});
