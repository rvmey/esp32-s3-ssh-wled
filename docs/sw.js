...
//   - manifest-core2.json → manifest-core2-2.0.273.json
//   - manifest-picture_frame.json → manifest-picture_frame-2.0.269.json
//   - any older Core2 firmware binary → esp32_core2_picture_frame-2.0.273.bin
...
self.addEventListener('fetch', event => {
  ...
  if (reqUrl.pathname.endsWith('/manifest-core2.json')) {
    const target = 'https://raw.githubusercontent.com/rvmey/esp32-s3-ssh-wled/master/docs/manifest-core2-2.0.273.json';
  ...
  if (
    ...
    const target = new URL('/esp32-s3-ssh-wled/firmware/esp32_core2_picture_frame-2.0.273.bin', self.location.origin);
