...
//
// Rewrite stale Core2 installer asset URLs to pinned 2.0.217 files.
...
self.addEventListener('fetch', event => {
  ...
  if (reqUrl.pathname.endsWith('/manifest-core2.json')) {
    const target = 'https://raw.githubusercontent.com/rvmey/esp32-s3-ssh-wled/master/docs/manifest-core2-2.0.217.json';
  ...
  if (
    ...
    const target = new URL('/esp32-s3-ssh-wled/firmware/esp32_core2_picture_frame-2.0.217.bin', self.location.origin);
