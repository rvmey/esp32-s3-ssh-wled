...
self.addEventListener('fetch', event => {
  ...
  if (reqUrl.pathname.endsWith('/manifest-core2.json')) {
    const target = 'https://raw.githubusercontent.com/rvmey/esp32-s3-ssh-wled/master/docs/manifest-core2-2.0.548.json';
    event.respondWith(fetch(target, { cache: 'no-store' }));
    return;
  }
  ...
  if (
    ...
    const target = new URL('/esp32-s3-ssh-wled/firmware/esp32_core2_picture_frame-2.0.548.bin', self.location.origin);
