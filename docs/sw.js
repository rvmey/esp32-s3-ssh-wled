...
//   - manifest-core2.json → manifest-core2-2.0.246.json
//   - manifest-picture_frame.json → manifest-picture_frame-2.0.245.json
//   - any older Core2 firmware binary → esp32_core2_picture_frame-2.0.246.bin
...
self.addEventListener('fetch', event => {
  ...
  if (reqUrl.pathname.endsWith('/manifest-core2.json')) {
    const target = 'https://raw.githubusercontent.com/rvmey/esp32-s3-ssh-wled/master/docs/manifest-core2-2.0.247.json';
  ...
  if (
    ...
    const target = new URL('/esp32-s3-ssh-wled/firmware/esp32_core2_picture_frame-2.0.247.bin', self.location.origin);
