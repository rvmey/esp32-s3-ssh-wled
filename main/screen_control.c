 ...
 *
 * Scale selection:
 *   Odd  scale (1, 3, 5): 8×8  font, pixel_scale = scale  (square chars)
 *   Even scale (2, 4, 6): 8×16 font, pixel_scale = scale/2
 *                          char_w = 8 * pscale, char_h = 16 * pscale   */
                              ...
                              int logical_w, int logical_h)
{
    ...
    int use_tall = (scale & 1) == 0;       /* 1 = use 8x16, 0 = use 8x8 */
    int pscale   = use_tall ? scale / 2 : scale;
    int cw       = 8 * pscale;
    int ch       = (use_tall ? 16 : 8) * pscale;
    ...
    int font_col = (rel_x % cw) / pscale;  /* 0..7  */
    int font_row = (rel_y % ch) / pscale;  /* 0..7 or 0..15 */

    if (use_tall) {
        /* 8×16: MSB of each byte is the leftmost pixel */
        return (s_font8x16[gi][font_row] & (0x80u >> font_col)) != 0;
    } else {
        /* 8×8: LSB of each byte is the leftmost pixel */
        return (s_font8x8[gi][font_row] & (0x01u << font_col)) != 0;
    }
}
...
void screen_draw_text(const char *text)
{
    ...
    int use_tall_ww = (s_font_scale & 1) == 0;
    int pscale_ww   = use_tall_ww ? s_font_scale / 2 : s_font_scale;
    int cw_ww       = 8 * pscale_ww;
    int ch_ww       = (use_tall_ww ? 16 : 8) * pscale_ww;
    int max_cols = lcd_w() / cw_ww;
    int max_rows = lcd_h() / ch_ww;
