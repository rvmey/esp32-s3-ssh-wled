...
/* IBM VGA 8×16 font subset for the pair screen (ASCII 0x20–0x7E). */
static const uint8_t s_qr_font8x16[95][16] = {
...
static void qr_render_char(uint8_t *buf, int buf_w, int buf_h,
                            int cx, int cy, char ch,
                            uint8_t fg_h, uint8_t fg_l)
{
    if (ch < 0x20 || ch > 0x7E) ch = '?';
    const uint8_t *glyph = s_qr_font8x16[ch - 0x20];
...
static void qr_render_string(uint8_t *buf, int buf_w, int buf_h,
                              int x, int y, const char *str,
                              uint8_t fg_h, uint8_t fg_l)
{
    while (*str) {
        qr_render_char(buf, buf_w, buf_h, x, y, *str, fg_h, fg_l);
        x += 8;
        str++;
    }
}

static void draw_pair_qr_screen(const char *pair_code)
{
    char url[96];
    ...
    uint8_t qr_data[qrcodegen_BUFFER_LEN_FOR_VERSION(6)];
    if (!qrcodegen_encodeText(url, qr_tmp, qr_data,
                               qrcodegen_Ecc_MEDIUM, 1, 6,
                               qrcodegen_Mask_AUTO, true)) {
        ESP_LOGE(TAG, "QR encode failed for pair URL");
        char fallback[64];
        snprintf(fallback, sizeof(fallback), "Pair code:\n%s", pair_code);
        screen_draw_text(fallback);
        return;
    }
    ...
    size_t buf_bytes = (size_t)buf_w * buf_h * 2;
    uint8_t *buf = malloc(buf_bytes);
    if (!buf) {
        ESP_LOGE(TAG, "QR pair screen: malloc(%u) failed", (unsigned)buf_bytes);
        char fallback[64];
        snprintf(fallback, sizeof(fallback), "Pair code:\n%s", pair_code);
        screen_draw_text(fallback);
        return;
    ...
    /* Calculate QR pixel scale so QR fits in the available height */
    int text_height = 36;
    int avail_h = buf_h - text_height;
    int avail_w = buf_w;
    int quiet = 4;
    int total_modules = qr_size + quiet * 2;
    int scale = avail_h / total_modules;
    if (scale > avail_w / total_modules)
        scale = avail_w / total_modules;
    if (scale < 1) scale = 1;

    int qr_px = total_modules * scale;
    int qr_ox = (buf_w - qr_px) / 2;
    int qr_oy = (avail_h - qr_px) / 2;
    if (qr_oy < 0) qr_oy = 0;

    /* Draw white background for QR area (including quiet zone) */
    for (int y = qr_oy; y < qr_oy + qr_px && y < buf_h; y++) {
        for (int x = qr_ox; x < qr_ox + qr_px && x < buf_w; x++) {
            int off = (y * buf_w + x) * 2;
            buf[off]     = 0xFF;
            buf[off + 1] = 0xFF;
        }
    ...
    /* Draw QR modules (dark = black on white) */
    int mod_ox = qr_ox + quiet * scale;
    int mod_oy = qr_oy + quiet * scale;
    for (int my = 0; my < qr_size; my++) {
        for (int mx = 0; mx < qr_size; mx++) {
            if (!qrcodegen_getModule(qr_data, mx, my))
                continue;
            int px0 = mod_ox + mx * scale;
            int py0 = mod_oy + my * scale;
    ...
    /* Draw text below QR code — white on black */
    int text_y = avail_h + 2;
    /* "Pair code:" label centered */
    const char *label = "Pair code:";
    int label_w = (int)strlen(label) * 8;
    qr_render_string(buf, buf_w, buf_h,
                     (buf_w - label_w) / 2, text_y,
                     label, 0xFF, 0xFF);
    /* Pair code value centered */
    int code_w = (int)strlen(pair_code) * 8;
    qr_render_string(buf, buf_w, buf_h,
                     (buf_w - code_w) / 2, text_y + 18,
                     pair_code, 0xFF, 0xFF);

    screen_draw_rgb565(buf, buf_w, buf_h);
    free(buf);
}
 ...
 * SIP account be administratively parked without erasing the account
 * credentials (e.g. to free internal RAM for something else). */
...
static bool mp3_queue_seek_relative(int32_t delta_ms, const char *reason)
{
    if (!s_mp3.active || s_mp3.radio) {
        ESP_LOGW(TAG, "mp3: %s ignored (%s)", reason ? reason : "seek",
                 s_mp3.radio ? "radio mode" : "no track active");
        return false;
    }
...
static void radio_close_stream(void)
{
    if (s_radio_http) {
        esp_http_client_close(s_radio_http);
...
static bool radio_open_stream(void)
{
    ...
    ESP_LOGE(TAG, "radio: too many redirects");
    return false;
}
...
static int radio_refill(unsigned char *buf, int max_bytes)
{
    ...
    while (total < max_bytes) {
        ...
        if (s_radio_icy_metaint > 0 && s_radio_icy_remaining == 0) {
            unsigned char meta_len_byte = 0;
            int n = esp_http_client_read(s_radio_http, (char *)&meta_len_byte, 1);
            if (n <= 0) break;
        ...
        int want = max_bytes - total;
        if (s_radio_icy_metaint > 0 && want > s_radio_icy_remaining)
            want = s_radio_icy_remaining;
        if (want <= 0) break;

        int n = esp_http_client_read(s_radio_http, (char *)(buf + total), want);
        if (n <= 0) break;
        total += n;
...
static void mp3_player_task(void *arg)
{
    ...
    while (true) {
        ...
        if (s_mp3.radio) {
            if (!s_radio_http || opened_token != s_mp3.play_token) {
                radio_close_stream();
                opened_token = s_mp3.play_token;
                bytes_left = 0;
                read_ptr = inbuf;
                screen_draw_text("Connecting to\nradio stream...");
                if (!radio_open_stream()) {
                    ESP_LOGW(TAG, "radio: connection failed, retrying in 3s");
                    screen_draw_text("Radio connection\nfailed — retrying");
                    vTaskDelay(pdMS_TO_TICKS(3000));
                    continue;
                }
                mp3_request_ui_refresh();
            }
        } else if (!fp || opened_token != s_mp3.play_token) {
            if (fp) fclose(fp);
            fp = fopen(s_mp3.file_path, "rb");
            opened_token = s_mp3.play_token;
            bytes_left = 0;
         ...
         * Radio streams read from HTTP, not SPI, so skip this stall. */
#if CONFIG_BT_ENABLED && CONFIG_BT_A2DP_ENABLE
        if (!s_mp3.radio && (s_bt.connecting || s_bt.discovering)) {
        ...
        if (bytes_left < (MP3_INPUT_BUF_BYTES / 2)) {
            ...
            size_t n;
            if (s_mp3.radio) {
                n = (size_t)radio_refill(inbuf + bytes_left,
                                         (int)((size_t)MP3_INPUT_BUF_BYTES - (size_t)bytes_left));
            } else {
                n = fread(inbuf + bytes_left,
                          1,
                          (size_t)MP3_INPUT_BUF_BYTES - (size_t)bytes_left,
                          fp);
            }
    ...
    if (s_mp3.radio) {
        ...
        if (s_radio_title && s_radio_title[0]) {
            if (strlen(s_radio_title) > 40) {
                ...
                strncpy(title_short, s_radio_title, sizeof(title_short) - 1);
                title_short[sizeof(title_short) - 1] = '\0';
            }
        } else {
...
static const pf_cmd_t s_pf_cmds[] = {
    ...
    { "sipenable", "sipenable", "true",  "Toggle whether the SIP phone client is allowed to run, without erasing the configured account. Default on. Pass 'on'/'off' or omit to toggle.", "\xF0\x9F\x93\xB5" /* 📵 */, NULL },
#endif
};
...
static void pf_sip_media_pump(void)
{
    ...
    if (s_media_hw_talking) {
        ...
        /* Drain (and discard) far-end RTP so the idle timer stays live — lets the
         * remote-hangup check below fire even while talking. */
        sip_rtp_drain_rx();
    } else {
    ...
    /* Hangup detection: linphone doesn't route the in-dialog BYE back over our
     * TLS connection, so detect a remote hangup by loss of inbound RTP (the
     * relay stops forwarding when the far end goes away). Checked in both talk
     * and listen — talk mode keeps the timer live via sip_rtp_drain_rx(). */
                              ...
                              void       *ctx)
{
    ...
    if (strcmp(s_trigger, "text") == 0) {
        ...
        if (!sip_on && sip_in_call()) {
            ...
            ESP_LOGI(TAG, "SIP enabled: %s", s_sip_enabled ? "on" : "off");
        }
#endif
